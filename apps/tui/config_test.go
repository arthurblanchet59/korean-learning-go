...
func TestDefaultConfigUsesBuildTimeAPIURL(t *testing.T) {
	previous := defaultAPIURL
	defaultAPIURL = "https://release.example.test"
	t.Cleanup(func() { defaultAPIURL = previous })

	if config := defaultConfig(); config.APIURL != defaultAPIURL {
		t.Fatalf("default config ignored build-time API URL: %#v", config)
	}
}
